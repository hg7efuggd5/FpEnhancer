    ...
    if norm_type == "min-max":
        if mask is not None:
            img = (img * 1.0 - img[mask > 0].min()) / (
                img[mask > 0].max() - img[mask > 0].min()).clip(1e-6, None)
        else:
            img = (img * 1.0 - img.min()) / (img.max() - img.min()).clip(
                1e-6, None)
    elif norm_type == "mean-std":
        if mask is not None:
            img = (img * 1.0 -
                   img[mask > 0].mean()) / img[mask > 0].std().clip(
                       1e-6, None)
        else:
...
def sensor_noise(img,
    ...
    p_perlin = 0
    for ii in [0, 1, 2]:
        p_perlin += 2.0**(-ii) * perlin(
            img.shape[::-1], frequency=2**ii, seed=809)
    ...
    if do_wet is True:
        ...
        img_n = np.rint(np.maximum(0, img_n - strength * p_perlin)).astype(
            np.uint8)
    else:
        img_n = np.where(blob_noise | (1 - mask), 255, img_n)
        img_n = np.rint(np.minimum(255, img_n + strength * p_perlin)).astype(
            np.uint8)
...
def GaussianNoise(src, means, sigma, percentage):
    ...
    for i in range(NoiseNum):
        ...
        NoiseImg[randX - 1:randX + 1, randY - 1:randY +
                 1] = NoiseImg[randX - 1:randX + 1, randY - 1:randY +
                               1] + random.gauss(means, sigma)
