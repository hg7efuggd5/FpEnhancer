...
LastEditTime: 2024-12-20 11:09:34
...
from loss.loss import CodeLoss, FinalLoss, Stage2_CELoss, Stage2_CodeLoss
from models.Enhancer import VQFormerEnhancer
...
def train(
    ...
    # ------------ select loss ------------ #
    criterion_rec = FinalLoss()
    criterion_code = CodeLoss(beta=cfg.MODEL.beta)
    criterion_s2_code = Stage2_CodeLoss()
    criterion_s2_ce = Stage2_CELoss()
    ...
    for epoch in range(num_epoch):
        ...
        for i, (imgs, target) in enumerate(pbar):
            ...
            probabilities, z_q, tar_encoding_indices, tar_z_q = model(
                imgs, train_feat=True)

            # --- loss
            ce_loss = criterion_s2_ce(probabilities, tar_encoding_indices)
            code_loss = criterion_s2_code(z_q, tar_z_q)

            total_loss = ce_loss + code_loss
            items = {
        ...
        with torch.no_grad():
            ...
            for i, (imgs, target) in enumerate(pbar):
                ...
                decoded_images, z_q, z = model(imgs)

                # --- loss
                rec_loss = criterion_rec(target, decoded_images)
                code_loss = criterion_code(z_q, z)

                total_loss = rec_loss + code_loss
                items = {
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-y",
        type=str,
        default="config_VQFormerenhancer",
        help="yaml name",
    )
    parser.add_argument(
        "--cuda-ids",
    ...
    if cfg.MODEL.name == "VQFormerEnhancer":
        model = VQFormerEnhancer(
            ...
            quant_conv_path=cfg.MODEL.quant_conv_path,
            codebook_path=cfg.MODEL.codebook_path,
            post_quant_conv_path=cfg.MODEL.post_quant_conv_path,
